 ...
 * @author 
 */
@WebServlet(name = "SortOrderController", urlPatterns = {"/SortOrderController"})
public class SortOrderController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<Order> listOrd = dao.sort(column, sortOrder);
            if (listOrd.size() > 0) {
                request.getSession().setAttribute("VIEW_ORDER_EMPTY", null);
                request.setAttribute("VIEW_ORDER", listOrd);
                url = SUCCESS;
            } else {
                request.getSession().setAttribute("VIEW_ORDER_EMPTY", "Chưa có sản phẩm nào cả .-.");
