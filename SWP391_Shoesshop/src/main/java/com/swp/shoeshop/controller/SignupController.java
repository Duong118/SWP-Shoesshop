...
public class SignupController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (valid == false) {
                request.setAttribute("SIGNUP_ERROR", "reCAPTCHA not verified");
            } else if (password.equals(rePassword) && account == null && mailPattern.matcher(email).find() && valid == true) {
            ...
            if (!mailPattern.matcher(email).find()) {
                request.setAttribute("SIGNUP_ERROR", "Invalid email format");
            }

            if (password.compareTo(rePassword) != 0) {
                request.setAttribute("SIGNUP_ERROR", "Passwords do not match!");
            }

            if (valid == true && account != null) {
                request.setAttribute("SIGNUP_ERROR", "The account name or email already exists!");
            }
