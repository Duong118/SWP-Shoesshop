    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (sizeIdParam == null || sizeIdParam.trim().isEmpty()) {
                request.setAttribute("ERROR_ADDTOCART", "Please select size before adding to cart");
            ...
            if (availableQuantity <= 0) {
                request.setAttribute("ERROR_ADDTOCART", "Products with this size are currently out of stock.");
                url = SUCCESS + Integer.toString(pid);
            } else if (buyQuantity > availableQuantity) {
                request.setAttribute("ERROR_ADDTOCART", "The requested quantity (" + buyQuantity + ") exceeds the available quantity (" + availableQuantity + ") for this size");
                url = SUCCESS + Integer.toString(pid);
            } else {
                Cart cart = new Cart();
                if (account != null) {
                ...
                if (totalQuantityAfterAdd > availableQuantity) {
                    request.setAttribute("ERROR_ADDTOCART", "The quantity in the cart (" + amountOfCartItem + ") + the quantity to add (" + buyQuantity + ") exceeds the available quantity (" + availableQuantity + ") for this size");
                    url = SUCCESS + Integer.toString(pid);
                } else {
                    if (account != null) {
