...
public class CategoryTypeController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<Category> listCategory = cateDAO.getByCateType(type);
            List<Product> listProductsByCateType = proDAO.getProductsByCateType(type);

            context.setAttribute("VIEW_CATEGORY", listCategory); //set list category lên application scope
            request.setAttribute("VIEW_PRODUCTS", listProductsByCateType);
            request.setAttribute("VIEW_PRODUCTS_JSON", JSONConverter.toJSON(listProductsByCateType));
            context.setAttribute("LIST_PRODUCTS", listProductsByCateType);
            url = SUCCESS;
